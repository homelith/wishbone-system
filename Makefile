...
#IMAGE_NAME     := homelith.sakuracr.jp/gowin-tools
...
# get gowin tools introduced shell on a container, suitable for Linux Desktop and WSL2 where GUI tools run on host X server directly.
docker:
...
# leave running console on current terminal, access localhost:13389 and then use credential username=guser and password=password
docker-xrdp:
	@if !(type docker > /dev/null 2>&1); then echo "command 'docker' not found." ; exit 1 ; fi
	@if [ $(BUILD_REQUIRED) -eq 1 ]; then echo "container image $(IMAGE_NAME):$(IMAGE_TAG) not found, try \`make docker-build\` or \`make docker-pull\` to get it." ; exit 1 ; fi
